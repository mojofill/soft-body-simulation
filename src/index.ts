...
const init = () => {
    ...
    for (let y = 0; y < h; y++) {
        ball_arr.push([]);
        for (let x = 0; x < w; x++) {
            const ball = new Ball(start_x + dist * x, start_y + dist * y, r, 0, m, 0, false, "white", ctx);
            ball.v = new Vector(start_v_x, start_v_y);
    ...
    for (let y = 0; y < ball_arr.length; y++) {
        for (let x = 0; x < ball_arr[y].length; x++) {
            ...
            if (ball_right !== null) spring1 = new Spring(curr_ball, ball_right, k, c, ctx);
            if (ball_up !== null) spring2 = new Spring(curr_ball, ball_up, k , c, ctx);
            if (ball_up_right !== null) spring3 = new Spring(curr_ball, ball_up_right, k, c, ctx);
            if (ball_down_right !== null) spring4 = new Spring(curr_ball, ball_down_right, k, c, ctx);

            if (spring1 !== null) springs.push(spring1);
            if (spring2 !== null) springs.push(spring2);
            if (spring3 !== null) springs.push(spring3);
            if (spring4 !== null) springs.push(spring4);
            
            objects.push(ball_arr[y][x]);
        }
    }

    const rect1 = new Rect(0.5 * canvas.width, 0.5 * canvas.height - 200, 1000, 20, 0, 1, 0, true, "white", ctx);
    const rect2 = new Rect(0.5 * canvas.width + 500, 0.5 * canvas.height + 1000, 20, 1000, 0, 1, 0, true, "white", ctx);
    const rect3 = new Rect(0.5 * canvas.width - 500, 0.5 * canvas.height + 1000, 20, 1000, 0, 1, 0, true, "white", ctx);
    const rect4 = new Rect(0.5 * canvas.width, 0.5 * canvas.height + 300, 1000, 20, 0, 1, 0, true, "white", ctx);
...
const loop = () => {
    ...
    for (const object of objects) {
        object.applyPhysics(<PhysicsObject[]> objects, time.dt, ctx);
        object.render();
    }
